...
\begin{enumerate}
    \item \textbf{Caracterización distribucional} (guion reproducible
    \texttt{run\_missing\_cue.py}): cuatro pistas de un token con su agente
    especialista (\texttt{pear} $\to$ \textit{pear}; \texttt{car} $\to$ \textit{car};
    \texttt{green} $\to$ \textit{apple}; \texttt{animal} $\to$ \textit{horse}),
    \textbf{30 corridas por método y pista} con semillas fijas (1000--1029, sembrando
    los generadores de Python y NumPy por corrida), reportando media, desviación
    estándar, rango, fracción de corridas con distancia exactamente cero y tiempo
    medio por corrida.
    El prototipo es determinista y aporta un único valor por pista.
    ...
    prototipo y objeto definido con sus distancias en el panel), como verificación de
    integración con corridas individuales.
\end{enumerate}
