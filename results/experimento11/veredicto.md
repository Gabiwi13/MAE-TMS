...
- La especificidad fuera de dominio (P3): con 40 consultas la diferencia entre la EHAM única y los especialistas es una consulta. Lo que sí aparece es que el directorio deja pasar dos consultas que el contenido rechaza (`a piano with black and white keys` → dog, `a thunderstorm with heavy rain` → car, en las diez semillas): el índice de texto es relativo (16 niveles sobre 300 rasgos, exp10) y acepta pistas que la memoria de contenido no contiene. Es la fuga de la limitación (a) del reporte. La doble compuerta de contenido (el destino debe contener la pista; columna «acepta con compuerta» del README, campo `acepta_compuerta` en `raw/*_o0.json`) la cierra en este banco: el protocolo acepta 2 de 40 en los cuatro cortes, las dos originales con palabras del vocabulario (`food`, `table`), que también acepta el contenido. Sobre las 171 reservadas (10 semillas, primer sorteo; `compuerta_reservadas.md`) la compuerta no quita ninguna respuesta: en N ≥ 50 rechaza 24–26 de 1710 filas (1.4–1.5 %), todas ruteos a un agente equivocado que ya no producían recall («a mare and foal in a pasture» → cow, «a cow swishing its tail at flies» → dog); en N = 25 rechaza 361 (21.1 %), también sin recall. Convierte silencios en rechazos explícitos, sin costo de cobertura.
