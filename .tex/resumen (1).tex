...
El sistema alcanza una precisión madura del 97.5\,\% con rechazo claro de lo
que no conoce, y el directorio transactivo se forma en tan solo 14 interacciones
...
también un \emph{directorio} de quién dentro del grupo es el experto en cada
dominio.
...
consultas viajan directamente al experto, sin intermediario).
...
cada uno responde con un score gated por containment (la activación media de la
proyección~\eqref{eq:proyeccion-AND}, anulada si falta soporte), el de mayor
score gana y ejecuta el recall; \emph{todos} los agentes y el TME registran el
...
$[68, 54, 42]$), y el score crudo de una proyección hetero-asociativa crece con
...
La precisión temprana fue de \textbf{96.2\,\%}, con rechazo de 2.5\,\%.
La precisión madura fue de \textbf{97.5\,\%} con lectura B1.
Sin la corrección B1, la precisión madura cae al 83.8\,\% (14 puntos): sin
normalizar, el agente más activo domina la puntuación sin importar el contenido
de la consulta.
La fidelidad entre fase temprana y madura fue del \textbf{96.2\,\%}.
El directorio cerró con conteos $[68, 54, 42]$ y entropía de
\textbf{1.558 bits} (98.3\,\% del máximo).
...
\textbf{$k = 14$ interacciones}; barajado, en $k \approx 26$ (rango $[18, 34]$,
5 semillas); bloqueado por dominio, hasta $k = 57$: el directorio no puede
enrutar expertise que no ha presenciado todavía.
Todas las condiciones con scoring corregido convergen al mismo estado final:
...
El routing de labels alcanza su mejor zona en $N = 200$ (96.2\,\%) y se degrada
en $N = 328$ (81.2\,\%) por saturación del eje contrario; por eso la zona de
...
\begin{table}[H]
    ...
    \begin{tabular}{llc}
        ...
        E32 / E64     & Cuantización binaria en 32 / 64 niveles           & 33.75 \\
...
En la dirección imagen $\to$ palabras, el routing visual acierta el
...
\textbf{16/17 = 94.1\,\%}.
En la dirección texto $\to$ imagen, los recalls visuales deben leerse como
...
\begin{figure}[H]
    ...
    tamaños de llenado $N$. A mayor cobertura, la memoria produce recalls con
...
Con un prototipo promedio repetido $N$ veces, el recall colapsa a un solo
