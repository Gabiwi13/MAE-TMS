...
Un \textbf{cue} es una función discreta $f : \{1, \ldots, n\} \to \{0, \ldots, m-1\}$,
representada como un vector $\mathbf{a} \in \{0, \ldots, m-1\}^n$.
La dimensión $n$ es el número de \emph{features} y $m$ es el número de valores posibles
por feature (el alfabeto).
La memoria se representa como una relación $\mathbf{R} \in \mathbb{N}^{n \times m}$,
donde $\mathbf{R}[i, j]$ cuenta cuántas veces el feature $i$ ha tomado el valor $j$ a
lo largo de todos los registros.

\subsubsection*{Operación de registro ($\lambda$)}

Dado un cue $\mathbf{a}$, se construye el patrón unitario:
\begin{equation}
...
La justificación de este mecanismo es central en la teoría: la memoria \emph{no almacena
...
frecuencias sobre los valores posibles del feature $i$.
La abstracción y el prototipo emergen de esta superposición estadística; no son
impuestos desde el exterior ni calculados explícitamente.

\subsubsection*{Operación de reconocimiento ($\eta$)}

Se dice que el cue $\mathbf{a}$ está \emph{contenido} en $\mathbf{R}$ si, para todo
feature $i$, la celda correspondiente tiene masa positiva: $\mathbf{R}[i, a_i] > 0$.
...
\begin{equation}
    ...
    \label{eq:pesos}
\end{equation}
Estos pesos reflejan cuán frecuentemente el feature $i$ ha tenido el valor $a_i$ en los
patrones registrados, y son la interfaz clave entre la memoria homo-asociativa y la
...
La recuperación de un patrón a partir de un cue $\mathbf{a}$ procede columna a columna.
Para cada feature $i$, se construye una distribución de probabilidad sobre $\{0,\ldots,m-1\}$
modulada por una gaussiana de desviación $\sigma \cdot m$ centrada en el valor $a_i$
(cuando se dispone del cue):
\begin{equation}
...
Esta indeterminación no es un defecto del modelo sino su propiedad constitutiva:
\emph{recordar es muestrear}, no leer un valor determinista.
Diferentes ejecuciones de $\beta$ sobre el mismo cue y la misma memoria pueden producir
resultados distintos.
El parámetro $\sigma$ controla la dispersión del muestreo: $\sigma = 0$ concentra la
masa en el cue exacto, valores mayores permiten explorar vecindades del espacio de
representación.
...
La entropía es la métrica de salud de la memoria: una memoria con entropía alta ha
acumulado distribuciones ricas; una con entropía baja está dominada por pocos valores
...
un dominio izquierdo con $n$ features y alfabeto de tamaño $m$, y un dominio
derecho con $p$ features y alfabeto de tamaño $q$.
...
para representar funciones parciales (features no definidos).

\subsubsection*{Registro hetero-asociativo}

Dados un cue izquierdo $\mathbf{a} \in \{0,\ldots,m-1\}^n$ y un cue derecho
...
Sea $\mathbf{a}$ un cue izquierdo con pesos por dimensión $\mathbf{w}$ provenientes de
...
todos los features del cue tienen soporte simultáneo en la memoria.
Los pesos $w'_i$ ponderan la suma por la frecuencia con que cada feature del cue
ha sido observado en la memoria homo-asociativa: features muy frecuentes en el cue
...
\subsubsection*{Asimetría fundamental de los dominios}

La memoria homo-asociativa de un dominio modela la \emph{distribución marginal} de ese
dominio: genera pesos $w_i$ que reflejan cuán típico es un cue dentro de ese espacio.
La memoria hetero-asociativa, en cambio, modela la \emph{distribución conjunta} de dos
dominios y, por construcción, \textbf{no genera pesos por feature} del dominio derecho.
Esta asimetría es constitutiva y justifica que cada agente en MAE-TMS requiera
\emph{exactamente cuatro} memorias: dos homo-asociativas (una por dominio, como
...
\begin{enumerate}
    ...
    ---incluyendo aquellos que no respondieron--- actualizan su directorio anotando quién
    fue el experto.
    ...
    coordinador central.
    El \textbf{rechazo explícito} ---cuando ningún miembro reconoce el dominio de la
    consulta--- es un comportamiento legítimo del grupo y evita que el sistema invente
    expertos inexistentes.
\end{enumerate}

La \textbf{fase temprana} se caracteriza por la dependencia de un mediador: los miembros
...
La métrica de salud del sistema transactivo es la \textbf{entropía del directorio}:
un directorio con entropía máxima $\log_2 K$ (siendo $K$ el número de agentes)
indica especialización perfectamente balanceada; una entropía baja señala colapso hacia
