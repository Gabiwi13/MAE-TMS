...
\textbf{precisión temprana del 90.0\%} y una \textbf{precisión madura del 93.5\%}
con la lectura B1 del directorio, con una fidelidad temprana-madura del 95.2\% y la
participación de \texttt{apple}, el agente históricamente sesgado, en \textbf{13.2\%}
(ideal 12.5\%).
En la dirección inversa, el hemisferio visual enrutó el \textbf{96.2\%} de las 656
imágenes de prueba con \textbf{dos falsos enrutamientos} (99.7\% de precisión sobre
las aceptadas; ambos, vistas del mismo objeto hacia la clase vecina) y una evocación
de dominio del \textbf{97.4\%}; su directorio alcanzó
el balance perfecto: \textbf{3.000 de 3.000 bits} de entropía.
Sobre objetos físicos nunca vistos (leave-one-object-out, diez pliegues) la cobertura
de punta a punta es del \textbf{89.2\%}, seis puntos por debajo de la de vistas nuevas
de objetos conocidos, con 2 falsos enrutamientos de 3280.
...
50\% al 90\%), la separabilidad del espacio latente del encoder (cuyo
reentrenamiento resolvió el sesgo hacia \texttt{apple}: 20\% $\to$ 13.2\%), y la
...
sesgo ``léxico'' hacia \texttt{apple} era en buena parte solapamiento latente del
encoder.
